...
public class SimilarityController {
    ...
    public boolean copyPasteGlobalSim(
            ...
            @RequestParam(value=SOURCE_SIM_NAME, required = true, defaultValue = EMPTY) String sourceSimName,
            @RequestParam(value=DESTINATION_SIM_NAME, required = true, defaultValue = EMPTY) String destinationSimName
        ) {

        if (null==destinationSimName || destinationSimName.isEmpty())
            destinationSimName = sourceSimName + COPY;
